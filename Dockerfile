...
# Install docker
RUN set -x \
  && curl -fSL "https://get.docker.com/builds/Linux/x86_64/docker-1.13.1.tgz" -o docker.tgz \
  && tar -xzvf docker.tgz \
...
RUN apt-get update \
	&& apt-get install -y unzip iptables lxc fontconfig libffi-dev build-essential git python-dev libssl-dev python-pip \
	&& rm -rf /var/lib/apt/lists/*
